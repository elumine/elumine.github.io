...
routeConfig = function($stateProvider, $urlRouterProvider) {
  $urlRouterProvider.when('/feed/article/:key/', '/feed/article/:key').when('/feed', '/feed/all').otherwise('/feed');
...
Article = (function() {
  function Article(data) {
    ...
    this.key = data.key, this.name = data.name, this.poster = data.poster, this.description = data.description, this.text = data.text, this.video = data.video, this.date = data.date, this.content = data.content, this.category = data.category;
    this.features = [];
    if (data.features) {
      ref = data.features;
  ...
  function ArticlesService(options) {
    ...
    DATA_STORAGE.articles.forEach((function(_this) {
      return function(articleData) {
        ...
        if (!_this.categories[articleData.category]) {
          _this.categories[articleData.category] = [];
        }
        return _this.categories[articleData.category].push(article);
      };
    })(this));
  }
...
angular.component(Main = (function() {
  ...
  Main.services = ['$state', 'articlesService'];

  function Main(options) {
    var k, ref, v;
    this.services = options.services, this.scope = options.scope;
    this.scope.categories = [];
    ref = this.services.articlesService.categories;
    for (k in ref) {
      v = ref[k];
      this.scope.categories.push(k);
    }
    this.scope.links = [
      {
        href: 'www.artstation.com',
        text: 'artstation'
      }, {
        href: 'www.github.com',
        text: 'github'
...
angular.component(Feed = (function() {
  ...
  function Feed(options) {
    ...
    this.category = this.services.$state.params.category || 'all';
    this.articles = options.services.articlesService.getArticlesByCategory(this.category);
  }
...
angular.component(Header = (function() {
  ...
  Header.services = ['$state', 'articlesService'];

  function Header(options) {
    var PROGRESS_BAR_VELOCITY_FACTOR;
    this.scope = options.scope;
    this.services = options.services;
    this.scope.progress = 0;
    this.scope.currentArticleIndex = 0;
    this.scope.currentArticle = null;
    PROGRESS_BAR_VELOCITY_FACTOR = 0.25;
    this.changeArticle();
    setInterval((function(_this) {
      return function() {
        _this.scope.progress += 0.01 * PROGRESS_BAR_VELOCITY_FACTOR;
        if (_this.scope.progress >= 1) {
          _this.scope.progress = 0;
          _this.scope.currentArticleIndex++;
          _this.changeArticle();
        }
  ...
  Header.prototype.changeArticle = function() {
    if (this.scope.currentArticleIndex > this.services.articlesService.articles.length - 1) {
      this.scope.currentArticleIndex = 0;
    }
    this.scope.currentArticle = this.services.articlesService.articles[this.scope.currentArticleIndex];
    return console.info(this.scope);
  };
