...
var MAKE_COMPONENT, MAKE_SERVICE, extend, include;
...
routeConfig = function($stateProvider, $urlRouterProvider) {
  $urlRouterProvider.when('/about', '/about').when('/projects', '/projects/').otherwise('/projects/');
...
Work = (function() {
  function Work(data, key, catKey) {
    ...
    this.key = key;
    this.catKey = catKey;
    this.name = data.name, this.poster = data.poster, this.description = data.description, this.text = data.text, this.video = data.video, this.date = data.date;
    this.features = [];
    if (data.features) {
      ref = data.features;
...
angular.service(WorksService = (function() {
  ...
  WorksService.prototype.getProjectsByCategory = function(cats) {
    var i, j, len, len1, ref, result, searchCat, selfCat;
...
angular.component(Navigation = (function() {
  ...
  function Navigation() {
    this.links = [new NavigationLink('projects/web', 'web'), new NavigationLink('projects/art', 'art'), new NavigationLink('projects/games', 'games'), new NavigationLink('projects/programming', 'programming'), new NavigationLink('about', 'about')];
  }
...
angular.component(RecentProjects = (function() {
  ...
  RecentProjects.services = ['data', '$interval'];

  function RecentProjects(options) {
    var cK, cV, pK, pV, ref, ref1;
    this.projectsService = options.services.data;
    this.intervalService = options.services.$interval;
    this.scope = options.scope;
    this.changeDelay = 500;
    this.projectsMap = [];
    ref = this.projectsService.categories;
    for (cK in ref) {
      cV = ref[cK];
      ref1 = cV.works;
      for (pK in ref1) {
    ...
    this.currentProject = this.projectsMap[0];
    this.intervalService((function(_this) {
      return function() {
        return _this.changePoster();
      };
    })(this), this.changeDelay);
  }

  RecentProjects.prototype.onPosterClick = function() {
    return window.location.hash = this.currentProject.projectUrl;
  };

  RecentProjects.prototype.changePoster = function() {
    var currentIndex, nextIndex;
    currentIndex = this.projectsMap.indexOf(this.currentProject);
    nextIndex = 0;
    if (currentIndex >= 0) {
      if (currentIndex < (this.projectsMap.length - 1)) {
        nextIndex = currentIndex + 1;
      }
    }
    return this.currentProject = this.projectsMap[nextIndex];
  };
...
angular.component(Projects = (function() {
  ...
  function Projects(options) {
    ...
    this.key = options.services.$state.params.key;
    this.filters = this.parseKey(this.key);
    this.displayables = this.worksService.getProjectsByCategory(this.hasKey() ? this.filters : null);
  }

  Projects.prototype.parseKey = function(key) {
    var filters;
    filters = [];
    switch (key) {
      case 'art':
        filters = ['2d-art', '3d-art'];
        break;
      default:
        filters = [key];
    }
